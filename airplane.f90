...
implicit none
private
public forces, torques, mass
real(dp) :: pi = 3.1415926535897931_dp
contains
function forces(vbody) result(f)
    ...
    f(1) = -d*cos(alpha) + l*sin(alpha) + thrust(vbody(1))
...
function torques(x, v, vbody, ang, ang_v) result(m)
    ...
    m(2) = pid_output(pid_pitch, pitch_setpoint, ang(2)) !- ang_v(2)
